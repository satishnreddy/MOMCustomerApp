...
public enum WebSocketError
{
     ...
     * *********************************************************** start ***************************************************
     *
     * WebSocketError for mswipe
     *
     * The below WebSocketError error code are defined by the mswipe,
     * addition to the existing error codes provided by the WebSocketError,
     * which we are using to define the errors.
     *
     */
    ...
    /**
     * SSL_EXCEPTION
     * This error will raised in two scenarios
     * 1) During the time when the connection is waiting for a response and in between if there is internet brackage it will throw up.
     * 2) In case connection to SSL is failed it will throw up this
     *
     */
    ...
    /**
     * SOCKET_EXCEPTION
     * this exception is triggere either if the internet is not available(ie wiif and mobile data are turned off)
     * or if its availabe the network this the mswipe server is down, this below check will handle this two instances
     *
     */
     ...
     * read time out, the way we test this is we have a service wm00, when we call this the service, this is written such that
     * it goes to time loop and does not send any response after 2- 3 min threshhold to the applicaiton and app wait for about 60 sec, this is arbitray
     * form device to device.
     *
     * when the connection is reading the response and during that time if there is an interuption in the conneciton,
     * the we show this message
     */
